...
// Content for this class is obtained from the public domain
// list at https://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types.
...
class MimeTypes {
	...
	constructor() {
		Object.keys(types).forEach((type)=>{
			let exts = (types[type] || "").split(/\s/g);
			exts.forEach((ext)=>{
				extensions[ext] = extensions[ext] || type;
			});
		});
	...
	getExtensionForType(type,defaultValue=null) {
		...
		return types[type] || defaultValue;
	}
	...
	getTypeForExtension(ext,defaultValue=null) {
		...
		ext = Path.extname(ext) || ext;
		if (ext[0]===".") ext = ext.slice(1);
		return extensions[ext] || defaultValue;
	}
}

module.exports = new MimeTypes;
